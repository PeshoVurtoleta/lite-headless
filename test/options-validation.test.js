...
// ---------------------------------------------------------------------------
// A3: drift guard -- every "create*" export validates, and there are 63 of them
// ---------------------------------------------------------------------------
test("A3: all 63 factories reject an unknown option key", () => {
    const factories = Object.keys(headless).filter((k) => k.startsWith("create"));
    assert.equal(factories.length, 63, "expected exactly 63 create* factories");
