...
// ---------------------------------------------------------------------------
// A3: drift guard -- every "create*" export validates, and there are 62 of them
// ---------------------------------------------------------------------------
test("A3: all 62 factories reject an unknown option key", () => {
    const factories = Object.keys(headless).filter((k) => k.startsWith("create"));
    assert.equal(factories.length, 62, "expected exactly 62 create* factories");
