...
- **Signal-node pool return (H-12).** lite-signal pools its nodes in a fixed-capacity registry (default 1024, fail-fast). `destroy()` *seals* every signal the factory created -- after the cleanup ledger has stopped every effect reading them, the pooled node is disposed and the read target swaps to a frozen stand-in -- so create/destroy churn runs indefinitely without exhausting the pool, while a destroyed handle keeps answering its final `open()`/`status()`/`value()` (writes stay no-ops). That includes nodes created after construction: kanban's per-column order signals go back on `removeColumn`, file-upload's per-entry `bytesLoaded`/`progress` on `removeEntry`/`clear` (the entry object keeps reading its final values), radio-group's per-item disabled signal on item detach. A consumer-supplied controlled signal is never disposed -- it's yours. Pinned by `test/signal-pool.test.js`, which churns every factory on a 256-node fixed registry.
...
- [`@zakkster/lite-signal`](https://www.npmjs.com/package/@zakkster/lite-signal) — **required peer**. Synchronous, zero-GC reactive primitives. The `open`, `value`, `status` signals come from here. `destroy()` returns every factory-owned node to lite-signal's pool, so the default fixed 1024-node registry bounds concurrent primitives, not lifetime churn; an app holding hundreds of live primitives at once (plus its own signals) should size its registry via `createRegistry` + `setDefaultRegistry` before constructing any.
