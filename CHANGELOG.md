...
- H6b: the `PaginationItem` ellipsis arm in the `./pagination` declare-module
  ...
  read `position` without a cast. This also corrects the declared return items
  of `buildItems` (declared in 1.4.0). Field-level only -- the name-level
...
- H-12: `destroy()` now returns every factory-owned lite-signal node to the
  registry pool (sealed via `src/_overlay/seal.js`). Previously the cleanup
  ledger stopped effects
